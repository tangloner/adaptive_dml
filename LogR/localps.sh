...
do
{
	...
	then
    	index=`expr 2222 - $i`
        python ./lr.py $ps $worker --job_name=ps --task_index=$index > ./result/$1"-"$2/ps"$index".txt
        echo "ps: "$index "done!"
	else
    	index=`expr 2222 - $1 - $i`
        python ./lr.py $ps $worker --job_name=worker --task_index=$index > ./result/$1"-"$2/worker"$index".txt
